...
public class Inventory : MonoBehaviour
{
    ...
    public void checkIfAllInventoryClosed()
    {
        ...
        for (int i = 0; i < canvas.transform.childCount; i++)
        {
            ...
            if (!child.activeSelf && (child.tag == "EquipmentSystem" || child.tag == "Panel" || child.tag == "MainInventory" || child.tag == "CraftSystem" || child.tag == "Shop"))
            {
                if (AllInventoriesClosed != null && i == canvas.transform.childCount - 1)
                    AllInventoriesClosed();
            }
            else if (child.activeSelf && (child.tag == "EquipmentSystem" || child.tag == "Panel" || child.tag == "MainInventory" || child.tag == "CraftSystem" || child.tag == "Shop"))
                break;
    ...
    public bool SetIfItemAllreadyExist(int itemID, int itemValue)
    {
        updateItemList();
        int stack;
        for (int i = 0; i < ItemsInInventory.Count; i++)
    ...
    public GameObject addItemToInventory(int id, int value)
    {
        for (int i = 0; i < SlotContainer.transform.childCount; i++)
        {
            if (SlotContainer.transform.GetChild(i).childCount == 0)
            {
                ...
                p.x = 14f;
                p.y = -16f;
                item.transform.GetChild(1).GetComponent<RectTransform>().localPosition = p;
