...
public class DragItem : MonoBehaviour, IDragHandler, IPointerDownHandler, IEndDragHandler
{
    ...
    public void OnPointerDown(PointerEventData data)
    {
        ...
        {
            //Debug.Log("onDrag name " + transform.parent.parent.parent.tag+"this item tag"+transform.tag);
            transform.GetComponent<ItemOnObject>().item.outbox = transform.parent.parent.parent.gameObject;
    ...
    public void createDuplication(GameObject Item)
    {
        ...
        GameObject duplication = GameObject.Find("PlayerGui").transform.GetChild(1).GetComponent<Inventory>().addItemToInventory(item.itemID, item.itemValue); //GameObject.FindGameObjectWithTag("MainInventory").GetComponent<Inventory>().addItemToInventory(item.itemID, item.itemValue);
    ...
    public void OnEndDrag(PointerEventData data)
    {
        ...
        if (data.button == PointerEventData.InputButton.Left)
        {
            ...
            if (newSlot != null)
            {
                ...
                if (secondItemRectTransform == null) { return; }
